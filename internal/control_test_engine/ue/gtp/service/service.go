...
func SetupGtpInterface(ue *context.UEContext, msg gnbContext.UEMessage) {
	firstGnbPduSession := msg.GNBPduSessions[0]
	firstPduSession, err := ue.GetPduSession(uint8(firstGnbPduSession.GetPduSessionId()))
	if firstPduSession == nil || err != nil {
		log.Error("[GNB][GTP] Aborting the setup of PDU Session ", firstGnbPduSession.GetPduSessionId(), ", this PDU session was not succesfully configured on the UE's side.")
		return
	}
	...
	ueGnbIp := net.ParseIP(msg.GnbIp)
	//upfIp := firstPduSession.GnbPduSession.GetUpfIp()
	//ueIp := firstPduSession.GetIp()
	msin := ue.GetMsin()
	...
	for _, gnbPduSession := range msg.GNBPduSessions {
		...
		pduSession.SetGnbIp(ueGnbIp)

		SetupVrfInterface(msin, pduSession, ue.TunnelMode)
	}
...
func SetupVrfInterface(msin string, pduSession *context.UEPDUSession, tunnelMode config.TunnelMode) {
	nameInf := fmt.Sprintf("val%s", msin)
	vrfInf := fmt.Sprintf("vrf%s", msin)

	ueIp := pduSession.GetIp()
	log.Info(fmt.Sprintf("[UE][GTP] Configuring vrf interface for UE %s and PDU session %d", ueIp, pduSession.GetPduSesssionId()))
	...
	cmdAddFar = []string{nameInf, "2", "--action", "2", "--hdr-creation", "0", fmt.Sprint(pduSession.GnbPduSession.GetTeidUplink()), pduSession.GnbPduSession.GetUpfIp(), "2152"}
	log.Debug("[UE][GTP] Setting up GTP Forwarding Action Rule for ", strings.Join(cmdAddFar, " "))
	if err := gtpTunnel.CmdAddFAR(cmdAddFar); err != nil {
		log.Fatal("[UE][GTP] Unable to create FAR ", err)
		return
	...
	cmdAddPdr := []string{nameInf, "1", "--pcd", "1", "--hdr-rm", "0", "--ue-ipv4", ueIp, "--f-teid", fmt.Sprint(pduSession.GnbPduSession.GetTeidDownlink()), pduSession.GetGnbIp().String(), "--far-id", "1"}
	log.Debug("[UE][GTP] Setting up GTP Packet Detection Rule for ", strings.Join(cmdAddPdr, " "))
	...
	tableId, _ := strconv.Atoi(fmt.Sprint(pduSession.GnbPduSession.GetTeidUplink()))
